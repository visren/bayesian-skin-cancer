...
ggplot(time.trends, aes(x = factor(year), y = Median, group=1)) +
    ...
    theme(text=element_text(size=16), plot.title=element_text(size=18, face="bold")) 
```
This is the posterior distribution of average risk per year, with 95% CI intervals. While the intervals are large (questionably so), the trend is consistent, with significant chance from 2003-2004, then staing relatively consistant from 2003 onwards
...
inequality <- tapply(risk.median, data.ordered$year, IQR)
