---
title: "final_proj_testing_chunked"
...
##############################################
#### Read in the data
dat <- read.csv(file="Spatio-temporal-modelling-tutorials-master/EnglandLUAdata.csv")
head(dat, n=3)
...
library(sf)
LA <- st_read("Spatio-temporal-modelling-tutorials-master/LocalAuthorities.shp")
...
```{r chain1_mini, eval=F, echo=F}
# This isn't part of the analysis, just some time tests to see how long it takes
set.seed(1)
...
```{r chain1, eval=F, echo=F}
set.seed(1)
chain1 <- ST.CARar(AR = 1, formula=Y~offset(log(E)) + PM25 + IMD, family="poisson", data=dat.ordered, W=W, burnin=200000, n.sample=2200000, thin=1000, verbose=FALSE)
```

```{r chain2, eval=F, echo=F}
set.seed(1)
chain2 <- ST.CARar(AR = 1, formula=Y~offset(log(E)) + PM25 + IMD, family="poisson", data=dat.ordered, W=W, burnin=200000, n.sample=2200000, thin=1000, verbose=FALSE)
```

```{r, chain3, eval=F, echo=F}
set.seed(1)
chain3 <- ST.CARar(AR = 1, formula=Y~offset(log(E)) + PM25 + IMD, family="poisson", data=dat.ordered, W=W, burnin=200000, n.sample=2200000, thin=1000, verbose=FALSE)
```

```{r convergence, cache=T}
set.seed(1)
